...
def getImageInfo(args, imageFullName=None, printOut=True):
    ...
    dockerPath = IMAGE_CONFIG[imageFullName]["dockerPath"].replace(
        "{FullName}", imageName)
    ...
    if json_tag:
        ...
        if printOut:
            ...
            if "imageRawSize" in json_tag:
                return {
                    "imageSize": imageSize,
                    "imageRawSize": json_tag['imageRawSize'],
                    "imageVersion": json_tag['imageVersion'],
        ...
        print(
            "!!Warning!! No tag name=%s is found for the image name=%s" %
            (tagName, imageName))
        sys.exit(1)
            ...
            for item in items:
                ...
                if key not in pkgs:
                    if not removeIt:
                        ...
                        print(
                            "!!Warning!! Removing non new primary pkg=%s is NOT supported" %
                            key)
                elif key in pkgs:
                    if removeIt:
                        del pkgs[key]
...
def create_container(contCmd, contName, dockerPath, bindOpt, force=False):
    ...
    jupyterOpt = "-p 8800:8800 -e NB_USER=%s -e HOME=%s -v %s:%s" % (
    ...
    createOpt = "-it -v %s:%s -w %s %s %s %s" % (
        pwd, pwd, pwd, jupyterOpt, bindOpt, secOpt)
...
def setup(args):
    ...
    if len(contCmds) == 0:
        print(
            "Found none of container running commands:",
            CONTAINER_CMDS,
            "; exit now")
...
def update(args):
    ...
    if latestUpdate > myImageUpdate and myImageDigest != latestDigest:
        print(
            "Update is available with the last update date=%s" %
            latestUpdate)
...
def getImageLabels(imageName, tag):
    ...
    url_inspect = "https://registry-1.docker.io/v2/%s/blobs/%s" % (
        repo, digest)
...
def main():
  ...
  source %s --sing ml-base:alma9-python39""" % (myScript, myScript)
    ...
    sp_listPackages = sp.add_parser(
        'listPackages', help=desc, description=desc)
    sp_listPackages.add_argument(
        'name',
        metavar='<ImageName>',
        help='Image name to list packages')
    sp_listPackages.set_defaults(func=listPackages)

    sp_printImageInfo = sp.add_parser(
        'printImageInfo',
        help='print Image Info',
        description='print out image size. last update date and SHA256 hash of the given image')
    ...
    sp_jupyter = sp.add_parser(
        'jupyter',
        help='run Jupyter with the container',
        description='run JupyterLab on the already created container/sandbox')
