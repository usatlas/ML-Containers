...
#
COPY shellWrapper-for-python2-I.sh shellWrapper-for-python2-nohome.sh /tmp/
RUN micromamba install -y jupyterlab click pyrsistent \
    && cd $prefix/bin \
    && sed -i -e '1r/tmp/shellWrapper-for-python2-I.sh' -e '0,/coding:/d' jupyter* \
    && sed -i -e '1r/tmp/shellWrapper-for-python2-nohome.sh' -e '0,/coding:/d' ipython2 ipython \
    && ln -s ipython ipython-nohome \
    && rm -f /tmp/shellWrapper-for-python2-I.sh /tmp/shellWrapper-for-python2-nohome.sh \
    && sed -i "1,3 s%${PWD}/python%/usr/bin/env python%" \
       $(file * | grep "script" | cut -d: -f1) \
